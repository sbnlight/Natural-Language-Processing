...
class BOWDataset(Dataset):
    ...
    def __getitem__(
        ...
        return (
            ...
            torch.tensor(length, dtype=torch.long), # 注意这里其实没用到 length 做 pack_padded_sequence，但在 EmbeddingBag 可能有用
...
class MultilayerPerceptronModel(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_classes: int, padding_index: int):
        ...
        # 使用 EmbeddingBag 处理变长/词袋输入，效率极高
        # mode='mean' 相当于求词向量平均
        self.embedding = nn.EmbeddingBag(vocab_size + 1, embed_dim, mode='mean', padding_idx=padding_index)
        
        self.fc1 = nn.Linear(embed_dim, hidden_dim)
        self.dropout = nn.Dropout(0.3)
        self.activation = nn.ReLU() # 题目要求尝试 ReLU, sigmoid, tanh
