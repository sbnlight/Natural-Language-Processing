...
def get_config(mode: str = "full"):
    ...
    if mode == "quick":
        ...
        # [Optimization]: This configuration is optimized for scoring
        # Key strategy: Slightly larger Vocab, moderate Model depth, slightly more Epochs
        return {
            ...
            # Vocab size: 4096 is a good balance point
            "vocab_size": 4096, 
            # Dimensions: 256 is sufficient for TinyStories
            "d_model": 256,
            # Layers: 6 is a standard small Transformer configuration
            "num_layers": 6,
            "num_heads": 8,
            "d_ff": 1024,
            "context_length": 512, # Must increase to 512 because we added Few-Shot examples
            
            # Training settings
            "pretrain_epochs": 1,  # Pretrain 1 epoch (TinyStories is large, 1 epoch is enough)
            "finetune_epochs": 3,  # Fine-tune 3 epochs
            "batch_size": 16,      # If VRAM is insufficient (e.g., OOM error), change to 8
            "learning_rate": 5e-4, # Slightly increase learning rate
        }
...
def grid_search_finetune(pretrained_model, tokenizer, config, device):
    """
    Manual grid search implementation without third-party libraries.
    """
    print("\n" + "=" * 60)
    ...
    # Tip: Learning rate affects results the most. Increase batch size if VRAM allows.
    search_space = [
        {"lr": 5e-5, "epochs": 3, "bs": 16},
        {"lr": 1e-4, "epochs": 3, "bs": 16},
        {"lr": 3e-4, "epochs": 3, "bs": 16}, # Usually 3e-4 works well
        {"lr": 5e-5, "epochs": 5, "bs": 16}, # Train for a few more epochs to see if it helps
    ]
    ...
    # Load validation set (only load once to save time)
    with open(config["qa_dev"]) as f:
        dev_data = json.load(f)
    dev_dataloader = create_qa_dataloader(
        data=dev_data,
    ...
    # Save initial weights of pretrained model to reset before each trial
    # This is very important! Otherwise you are fine-tuning on top of the previous trial
    original_model_state = copy.deepcopy(pretrained_model.state_dict())

    for i, params in enumerate(search_space):
        print(f"\n[Trial {i+1}/{len(search_space)}] Testing params: {params}")
        
        # Restore pretrained weights
        pretrained_model.load_state_dict(original_model_state)
        
        # Instantiate QA model
        qa_model = TransformerForMultipleChoice(
            transformer_lm=pretrained_model,
            hidden_size=pretrained_model.d_model,
            num_choices=4,
        ...
        # Dynamically create training config
        train_dataloader = create_qa_dataloader(
            data=train_data,
            tokenizer=tokenizer,
        ...
        current_train_config = TrainingConfig(
            ...
            max_grad_norm=1.0,
            device=device,
            log_interval=1000, # Reduce log output
        )
        ...
        print(f"--> Trial {i+1} Accuracy: {acc:.2%}")
        ...
        if acc > best_acc:
            ...
            best_model_state = copy.deepcopy(qa_model.state_dict())

    print("\n" + "=" * 60)
    print(f"BEST RESULT: Accuracy {best_acc:.2%} with params {best_params}")
    print("=" * 60)

    # Restore best model and return
    final_model = TransformerForMultipleChoice(
        transformer_lm=pretrained_model, # Just for initializing structure
        hidden_size=pretrained_model.d_model,
        num_choices=4,
        pooling="last"
    ).to(device)
...
def main():
    ...
    # Step 4: Fine-tune for QA (Grid Search)
    qa_model = grid_search_finetune(pretrained_model, tokenizer, config, device)
