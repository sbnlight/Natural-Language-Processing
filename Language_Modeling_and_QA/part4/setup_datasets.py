...
def main():
    ...
    # Check if SQuAD already exists to avoid overwriting standard fixtures
    train_path = FIXTURES_DIR / "squad_train.json"
    val_path = FIXTURES_DIR / "squad_dev.json"
    
    if train_path.exists() and val_path.exists():
        print("\n" + "=" * 60)
        print("SQuAD datasets already exist in fixtures.")
        print("Skipping download to preserve original order (as per instructions).")
        print("=" * 60)
        squad_paths = (train_path, val_path, FIXTURES_DIR / "squad_test.json")
    else:
        # Download only if SQuAD datasets don't exist
        print("\nSQuAD datasets not found. Downloading and generating...")
