...
def softmax(x: Tensor, dim: int = -1) -> Tensor:
    ...
    # Handle fully masked rows (all -inf)
    # If max is -inf, replace with 0 to prevent NaN (since -inf - -inf = NaN)
    # This ensures that exp(x - x_max) results in 0 for all elements
    x_max_safe = x_max.masked_fill(x_max == float('-inf'), 0.0)
    
    exps = torch.exp(x - x_max_safe)
    exps_sum = exps.sum(dim=dim, keepdim=True)
    
    # Avoid division by zero if all elements are -inf (sum is 0)
    # In this case, we want the output to be uniform (or zero), masking avoids division by zero
    exps_sum = exps_sum.masked_fill(exps_sum == 0.0, 1.0)
